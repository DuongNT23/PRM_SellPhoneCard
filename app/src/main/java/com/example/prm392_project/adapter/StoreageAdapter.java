...
//                            productDAO.getListProduct();
                          // String IdStorage =
                           // Long id = Long.parseLong(IdStorage);
                            IdStorage = getAdapterPosition()+1;
                            Storage storage = storageDAO.getStorageById(IdStorage);
                            storage.setDelete(true);
                            LocalDateTime now = LocalDateTime.now();
                            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
                            String formattedDateTime = now.format(formatter);
                            Timestamp timestamp = Timestamp.valueOf(formattedDateTime);
                            storage.setDeletedAt(timestamp);
                            storageDAO.delete(storage);
                            Toast.makeText(context, "Delete kho hang thanh cong", Toast.LENGTH_SHORT).show();
